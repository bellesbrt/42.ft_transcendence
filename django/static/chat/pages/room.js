function popupAlert(title, message) {
	...
	popupBackground.innerHTML = '<div class="popup-background" id="popup-background"></div>';
	popup.innerHTML = `
		<div class="popup">
	...
	`;
	document.body.append(popupBackground, popup);
	const removePopup = () => {
		popup.remove();
		popupBackground.remove();
	};
	document.getElementById('close-popup').addEventListener('click', removePopup);
	document.getElementById('popup-background').addEventListener('click', removePopup);
}
...
function send_message(room_id, sender, message) {
	...
	const socketUrl = `${protocol}//${window.location.hostname}${port}/ws/chat/${room_id}/`;
	const socket = new WebSocket(socketUrl);
	socket.onopen = () => socket.send(JSON.stringify({ 'message': message, 'sender': sender, 'username': 'System Info' }));
}